...
class Keysight_86100D_Oscope (kf.VisaManager):
        ...
        while col < 751:
            ...
            while row < 521:
                ...
                if point:  # point has hits
                    ...
                    y.append(round((row / 521.0 * 8), 3))  # scaled y location
                row += 1
                n += 1
            col += 1
    ...
    def magnifier(self, xScale=None, yScale=None, channel=None):
        ...
        if yScale is not None and channel is not None:
            offset = self.FlexDCA.write(':CHANnel'+str(channel)+':YOFFset?')
            try:
                self.FlexDCA.write(':CHANnel'+str(channel)+':YOFFset '+str(offset))
    ...
    def acquire_eye(self, channel, scale=None):
        ...
        kf.waveform_acquisition(self.FlexDCA, 50)
